...
export default function DocumentPanel({ projectId }: { projectId: string }) {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try { setResults(await invoke("search_knowledge", { query: q, projectId }) as SearchResult[]); } catch {}
    setSearching(false);
  };
  ...
  const handleAsk = async () => {
    ...
    const unlisten = await listen<{ delta: string }>("polish-stream", (event) => {
      streamed = true; setAnswer((prev) => prev + event.payload.delta);
    });
    try {
      const result = await invoke<string>("ask_knowledge", { question: q, projectId, kbIds: searchKbIds });
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
            {results !== null ? (results.length === 0 ? (
              ...
              <div style={{ textAlign: "center", color: "var(--color-muted)", marginTop: "var(--space-xl)", fontSize: "13px" }}>当前知识库暂无文献，上传 PDF 开始</div>
            ) : (
              docs.map((doc) => (
